...
/**
 * Derives a symmetric presence key from an ECDH shared secret.
 *
 * Uses keyed BLAKE2b-256 with domain separator `encra:presence:v1` so the key is
 * cryptographically isolated from message keys derived from the same ECDH output.
 * Both parties derive the same key independently — no additional exchange needed.
 *
 * @param sharedSecret - 32-byte output of `deriveSharedSecret`.
 * @returns 32-byte symmetric presence key.
 * @throws {InvalidKeyError} If sharedSecret is not 32 bytes.
 * @example
 * const secret = await deriveSharedSecret(myPrivKey, theirPubKey)
 * const presenceKey = await derivePresenceKey(secret)
 */
